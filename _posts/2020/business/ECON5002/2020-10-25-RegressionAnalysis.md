...
为了计算估计量，我们只假设\\(x _i, i = 1 \cdots n\\) 中的变量
...
学习如何将OLS作为一种计算工具，将（希望）有助于OLS作为估计量的使用和解释。
...
其中\\(\beta _0\\) 和\\(\beta _1\\)是（未知）总体*截距*和*斜率*参数。
...
示例结果\\( \lbrace x _1, \cdots , x _n \rbrace \\)的值不完全相同。
...
如果\\(y = log(wage)\\)、\\(x = education\\)和\\(u = innate ability\\)，假设SLR.4意味着先天能力的平均水平不依赖于教育。
...
其中\\( x = \lbrace x _1, \cdots, x _2 \rbrace \\)（示例值）。

But \\( \sigma ^2\\) is unknown! And \\(u _i s\\) are not observable!  
但是\\( \sigma ^2\\)是未知的！和\\(u _i s\\)是不可见的！
...
如果\\( \hat{\sigma} ^2\\) 插入了公式\eqref{varianceEstimator1}和\eqref{varianceEstimator2}，我们就得到了方差的估计。

- Similarly, the **standard errors (se)** of \\(\hat{\beta} _0 \\) and \\(\hat{\beta} _1 \\).  
类似地，\\(\hat{\beta} _0 \\)和\\(\hat{\beta} _1 \\)的**标准错误（se）**。
...
假设SLR.1-SLR.4有助于证明\\(\hat{\beta} _0\\)，\\(\hat{\beta} _0\\)是无偏的。
...
因为\\( E(y _i \| x _i) = \beta _0 + \beta _1 x _i \\) (SLR.4)，估计\\(\hat{\beta} _0\\)和\\(\hat{\beta} _1\\)意味着估计CEF。
...
均值和方差指的是随机变量！这里我们考虑的是OLS估计，而不是估计。
...
设\\(x _i\\)一个二进制变量（要么\\(x _i = 1 \text{ or } x _i = 0\\)。
...
我们感兴趣的是\\(x _i\\)对\\(y _i (x _i)\\)的因果关系。两种**潜在**结果之间的差异：
...
等式\eqref{observedOutcome}可以被重新为
...
其中 \\( \tau _{ate} = \alpha _1 - \alpha _0\\)，而且
...
OLS估计量对于\\(\alpha\u0\\)和\\(\tau{ate}\\)是无偏的！
...
随机赋值意味着\\(E[u _i \| x _i]  = 0 \\)。我们的结论是 \\( \hat{\tau} _{ate} \\)是无偏的。
...
*IQ*是智商得分（在人群中，*IQ*得分的平均值为100，标准差为15。）
...
主要对\\(\beta _1\\)感兴趣，但\\\(\beta _2\\)也有兴趣。在方程中加上IQ，就从误差项中去掉了。
...
在有两个解释变量的模型中，关于u如何与\\(x _1 \\)和\\(x _2 \\)相关的关键假设是
...
那么\\( E (y \| x _1, x _2 ) = \beta _0 + \beta _1 x _1 + \beta _2 x _2\\)和
...
其他因素，如工作经验和“激励”也是美国的一部分。激励很难衡量。测量体验更容易：
...
\\( \beta _0\\)是**截距**，\\( \beta _1\\)是与\\( x _1 \text{, } \beta _2\\)相关的参数，依此类推。

Contains *k* + 1 (unknown) population parameters. We call \\( \beta _1, \cdots, \beta _k \\) the **slope parameters**.  
包含*k*+1（未知）总体参数。我们称\\(\beta\u 1、\cdots、\beta\u k\\)为**斜率参数**。
...
\\( \beta _1\\) 衡量由于\\(x _1 \\)增加一个单位，持有\\(x _2 \\)固定不变而导致的*y*的预测变化。
...
其中\\( \sum _{i=1}^n x _1 \widetilde{r} _i =0 \\)由构造。
...
将y在\\( x _1\\)上和一个常数或仅在\\([x _1, \widetilde{r}]\\)上和一个常数上回归得到\\( x _1\\)系数的相同估计。
...
如果\\( x _2\\)对*y*的部分效应是正的，那么\\( \widetilde{\beta} _2 > 0\\)和\\( x _1\\)和\\( x _2\\)在样本中是正相关的，所以\\( \widetilde{\delta} > 0 \\)，那么
...
我们从人群中随机抽取了一个大小为n的样本，\\( \lbrace [x _{1i}, x _{2i}, \cdots, x _{ki}, y _i]: i = 1, \cdots,n \rbrace \\)
...
由于样本量小或绘图运气不好，也可能出现完全共线。e、 g.*educ/IQ*=常数。

Assumption MLR.3 can only hold if \\( n > k + 1 \\).  
假设MLR.3只能在\\(n>k+1\\)时成立。
...
即使在相同的收入水平下，家庭对子女教育的兴趣和关注程度也不尽相同。
...
当一个变量应该包含在多元回归中时，将其忽略可能会导致**忽略变量偏差**。
...
但是假设我们省略了\（x\u 2 \），使用简单的回归*y*在\（x\u 1 \）上。
...
偏差\\(\left（\widetilde{\beta}u 1\\x\right）=E\left（\widetilde{\beta}u 1\\x\right）-\beta}u 1=\beta}u 2\widetilde{\delta}u 1\\)
...
可以为任何\\(\lbrace（x{1i}，x{2i}），i=1，\cdots，n\rbrace\\)计算偏差，但是如果\\(i n\\rightarrow\infty\\)，偏差将接近
...
简单回归估计在两种情况下是无偏的（对于给定的结果\\(\lbrace（x{1i}，x{2i}），i=1，\cdots，n\rbrace\\)：

1. \\( \beta _2 = 0 \\) ( \\(x_2\\) does not appear in the population model),  
1\\(\beta\u 2=0\\)（\\(x\u 2\\)未出现在总体模型中），


2. \\( \widehat{Corr} (x _{1i}, x _{2i}) = 0 \\) (in the sample), entailing \\( \widetilde{\delta} _1 = 0 \\)  
2\\(\widehat{Corr}（x{1i}，x{2i}）=0\\)（在示例中），包含\\(\widetilde{\delta}u1=0\\)
...
错误的条件方差ui不随\\(x{1i}，x{2i}，\cdots，x{ki}\\)中的任何一个而改变：
...
在假设MLR.1到MLR.5的情况下，以样本中解释变量的值为条件，
...
当\\( x _{1i} \\)和\\( x _{2i} \\)相关时，\\( R _1^2 > 0 \\)，和
...
在假设MLR.1到MLR.5下，OLS估计量\\(\hat{\beta}u 0、\hat{\beta}u 1、\cdots、\hat{\beta}u k\\)是**最佳线性无偏估计量（blue）**。

- Best: smaller variance  
最佳：方差较小

- Linear: can be expressed as a linear function of { \\(y _1, \cdots, y _n\\)},  
线性：可以表示为{\\(y\u 1，\cdots，y\u n\\)}的线性函数，
...
上大学的因果关系是艾米10美元，本4美元。平均因果效应为7美元。
...
col是大学的缩写。类似地，\\(Pr（col\| Ben）=0.375\\)。
...
让*col*表示一个二进制变量，对于大学毕业生取1，否则取0。从表1我们得到
...
8.25美元不是上大学的*平均因果效应*（ACE），而是人口中实际工资的观察差异。
...
在考试成绩高的学生中，有3/4的学生成绩差，1/4的学生成绩差。
...
在考试成绩低的学生中，1/4的学生是“阿美”，3/4的学生是“本”。
...
如果*col*改变时*u*和*hscore*保持不变，*col*对*wage*的因果关系可以计算为
...
由于\（x\u 1\）、保持\（x 21\）和*u*常数的变化而引起的*y*的变化，其中
...
我们可以把12:3%看作是对第一年工作经验的大致回报{基本上是从劳动力开始的。
...
但只有不到2%的观察结果显示exper>16，所以不用太担心。
...
\\(x\u 1\\)对y的部分效应（PE）为\\(\beta\u 1\\)，而\\(x\u 2\\)对y的部分效应（PE）为\\(\beta\u 2\\)。
...
有时，人们很自然地认为，一个变量（比如教育）的部分影响可能取决于另一个变量（比如智力）的水平。

- We add an **interaction term**, \\(x _1 x _2\\), to the usual model:  
我们在通常的模型中添加了一个**交互项**，\\(x\u 1 x\u 2\\)：
...
保持\（x\u 2\\)（和*u*）固定，\（x\u 1\\)对*y*的部分影响现在是
...
所以\\(x\u 1\\)的效应取决于\\(x\u 2\\)，除非\\(\beta\u 3=0\\)
...
\\(\beta\u 1\\)将被解释为对那些智商为0的人额外接受一年教育的影响。
...
如果回归数不同，使用\\(R^2\\)比较非嵌套模型可能不公平。

- The adjusted \\(R ^2\\) can be helpful in these cases.  
在这些情况下，调整后的\\(R^2\\)可能会有所帮助。
...
我们认为\\(R^2\\)是使用*SSR/n*来估计*Var（u）*，*SST/n*来估计*Var（y）*。
